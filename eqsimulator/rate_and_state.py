...
class RateStateFaultPatch(object):
    ...
    def initial_state(self, d_dot_0, theta_0, sigma_0, initial_shear_stress=None):
        ...
        if initial_shear_stress is None:
            ...
            self.shear_stress = np.float64(
                    initial_shear_stress * self.mu_0 * sigma_0
                    )
        ...
        if self.record_history:
            ...
            self._stress_drop_history = [0.]
            self._event_timings = [0.]
            self._coseismic_displacements = [0.]
    ...
    def state_1(self, t1_2=None):
        ...
        if self.record_history:
            ...
            self._event_timings.append(self._time[-1])
            # ----------------------------------
    ...
    def update_history(self, duration):
        total_time = np.linspace(
            self._time[-1], self._time[-1] + duration, N_POINTS_TIME_SERIES
        )
        ...
        self._shear_stress_history.extend(
                list(
                    self._shear_stress_history[-1] + self.shear_stress_rate * delta_time
                    )
                )
        self._normal_stress_history.extend(
                list(
                    self._normal_stress_history[-1] + self.normal_stress_rate * delta_time
                    )
                )
        if self.state == 2:
            self._coseismic_slip_history.extend(
                    list(
                        self._coseismic_slip_history[-1] + delta_time * self.d_dot_EQ
                        )
                    )
        elif self.state == 3:
            self._coseismic_slip_history.extend(
                    list(
                        self._coseismic_slip_history[-1] + delta_time * self.d_dot
                        )
                    )
        else:
            self._coseismic_slip_history.extend(
                    list(
                        self._coseismic_slip_history[-1] + delta_time * 0.0
                        )
                    )
        ...
        if duration == 0.0:
            ...
            if self.state == 0:
                ...
                if self.d_dot < 0.9 * self.get_steady_state_slip_speed():
                    ...
                    print('Go back to state 0: Friction = {:.2e}MPa, Driving force = {:.2e}MPa'.format(self.friction/1.e6, self.shear_stress/1.e6))
                    self.state = 0
            elif self.state == 2:
    ...
    def newton_fprime(self, t):
        ...
        projected_normal_stress_MPa = projected_normal_stress / 1.e6
        sigma_0_MPa = self.normal_stress / 1.e6
        c1 = self.alpha / self.b
        c2 = 1. + self.alpha / self.b
        if self.normal_stress_rate == 0.:
            theta_prime = 1.
        elif projected_normal_stress > 0.:
            theta_prime = (
                    1. / c2
                    -
                    c2 * 1.e-6 * abs(sigma_0_MPa)**c1
                    *
                    projected_normal_stress_MPa**(-c2)
                    *
                    (
            ...
            print("Unexpected", self.normal_stress, self.normal_stress_rate, t,
                    c1)
            return 0.
        ...
        fprime = (
                ...
                -
                self.normal_stress * self.a * theta_prime / theta_t**2
                *
                1. / (theta_t - c1 * self.normal_stress_rate / self.normal_stress)
                )
        return fprime
    ...
    def find_t3_1(self):
        """
        Patches in state 3 are creeping. If the critical stiffness evolves above the patch stiffness,
        then frictional instability is possible. Since a creeping patch is at steady state, it ends up
        in state 1 when the stiffness condition is satisfied.
        """
    ...
    def find_tX_3(self):
        """
        Patches in state 0 or 1 are unstable. If the critical stiffness evolves below the patch stiffness,
        then the patch becomes stable and starts creeping. When the stiffness condition is satisfied, the patch
        ends up in state 3.
        """
        # critical time at which critical stiffness is equal to patch stiffness
        if self.normal_stress_rate == 0.0:
    ...
    def find_t3_V_update(self, alpha=10.0):
        # ---------------------------------------------------------------------------
        if self.shear_stress_rate == 0.0:
            return np.finfo(np.float64).max
        characteristic_time = (
        ...
        return max(2.0e-5, alpha * characteristic_time)

    def find_t3_tau_update(self, alpha=1.0):
        if self.shear_stress_rate == 0.0:
            return np.finfo(np.float64).max
...
class RateStateFault(object):
    def __init__(
        ...
        for i in range(self.n_patches):
            self.fault_patches[i].ktau_tectonic = np.float64(
                -1.0
                * (
            ...
            if ktau_tectonic is not None:
                self.fault_patches[i].ktau_tectonic = ktau_tectonic
            self.fault_patches[i].ksigma_tectonic = np.float64(
                -1.0
                * (
            ...
            self.fault_patches[i].k = np.float64(abs(self.Kij_shear[i, i]))
            self.fault_patches[i].update_H()
            if self.fault_patches[i].tectonic_stressing_rate == 0.0:
                # tectonic slip speed is imposed
                self.fault_patches[i].shear_stress_rate = (
                    self.fault_patches[i].tectonic_slip_speed
                    * self.fault_patches[i].ktau_tectonic
                )
                self.fault_patches[i].normal_stress_rate = (
                    self.fault_patches[i].tectonic_slip_speed
                    * self.fault_patches[i].ksigma_tectonic
                )
                print(
                    "Tectonic stressing rates: shear rate = {:.2e}MPa/yr, normal rate = {:.2e}MPa/yr".format(
                        self.fault_patches[i].shear_stress_rate
                        * 3600.0
                        * 365.0
                        * 24.0
                ...
                # tectonic stress rate is imposed
                self.fault_patches[i].shear_stress_rate = (
                    self.fault_patches[i].tectonic_stressing_rate
                    * self.fault_patches[i].p[0]
                )
                self.fault_patches[i].tectonic_slip_speed = (
                    self.fault_patches[i].tectonic_stressing_rate
        ...
        for i in range(self.n_patches):
            if self.fault_patches[i].k > self.fault_patches[i].critical_stiffness():
                # stable patch (creeping)
                self.fault_patches[i].stable = True
                self.fault_patches[i].state = 3
                self.fault_patches[i].d_dot = self.fault_patches[i].tectonic_slip_speed
                self.fault_patches[i].set_theta_to_steady_state()
                self.fault_patches[i].set_friction_to_steady_state()
                self.fault_patches[i].shear_stress = self.fault_patches[
                    i
                ].friction  # stable steady state
                self.fault_patches[i].shear_stress_history[0] = self.fault_patches[
                    i
                ].shear_stress
                for j in range(self.n_patches):
                    # steady motion on patch i redistributes stress:
                    self.fault_patches[j].shear_stress_rate += (
                        self.fault_patches[i].tectonic_slip_speed * self.Kij_shear[i, j]
                    )
                    self.fault_patches[j].normal_stress_rate += (
                        self.fault_patches[i].tectonic_slip_speed
                        * self.Kij_normal[i, j]
                    )
                print("Patch {:d} is stable.".format(i))
                print(
                    "Shear stressing rate = {:.2f}".format(
                        self.fault_patches[i].shear_stress_rate
                    )
                )
